...
try {
    ...
    // CORRECTED: Total Employees - All employees regardless of status
    ...
    // CORRECTED: Active/Approved Employees - Only those with approved user accounts
    $active_emp_query = $conn->query("
        SELECT COUNT(*) as count 
        FROM employees e 
        INNER JOIN users u ON e.user_id = u.id 
        WHERE u.is_approved = 1 AND u.role = 'employee'
    ");
    ...
    $recent_leaves = $conn->query("SELECT lr.*, e.full_name as employee_name FROM leave_requests lr LEFT JOIN employees e ON lr.employee_id = e.id ORDER BY lr.created_at DESC LIMIT 10")->fetchAll(PDO::FETCH_ASSOC);
    ...
    // ADDITIONAL: Pending Approval Employees (for better insight)
    $pending_approval = $conn->query("
        SELECT COUNT(*) as count 
        FROM employees e 
        INNER JOIN users u ON e.user_id = u.id 
        WHERE u.is_approved = 0 AND u.role = 'employee'
    ");
...
/* Statistics Cards - Icon Higher and Smaller */
.stat-card {
...
<!-- Main Statistics - Updated with proper differentiation -->
<div class="row g-4 mb-4">
    ...
    <div class="col-xl-3 col-md-6">
        <div class="stat-card">
            ...
            <div class="stat-number" id="pendingEmployees"><?php echo $dashboardData['pending_employees']; ?></div>
...
async function updateDashboardData() {
    ...
    try {
        ...
        updateElement('pendingEmployees', data.pending_employees); // Updated
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Enhanced hover effects for quick actions
